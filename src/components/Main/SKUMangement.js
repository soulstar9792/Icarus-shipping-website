...
function SKUManagement() {
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/auth/update-sku/${user._id}/${editForm._id}`,
        { skuData: editForm },
        { headers: { "Content-Type": "application/json" } }
      );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/auth/delete-sku/${id}`,
        {},
        { headers: { "Content-Type": "application/json" } }
      );
      await getSKU();
                  ...
                  {editingId === item._id ? (
                    <>
                      ...
                      <td>
                        ...
                        >
                          <FaSave size={20} className="text-green-500" />
                        </button>
                      </td>
                    </>
                  ) : (
                    <>
                      <td className="border-b  border-custom-border px-3 py-4 text-sm md:text-base text-white">
                        {item.sku}
                      </td>
                      ...
                      <td>
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="p-2 hover:bg-gray-700 rounded-full"
                          >
                            <FaPencilAlt size={20} className="text-blue-500" />
                          </button>
                          <button
        ...
        <div className="flex gap-8 justify-center">
          ...
          >
            {uploadLoading ? (
              ...
              <>
                <FaUpload size={24} /> Upload CSV File
              </>
            )}
            <input
              type="file"
