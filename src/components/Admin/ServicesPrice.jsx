  ...
  const handleCostChange = async (userId, service, costType, value) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:5000/api/orders/service-price/${userId}`,
        {
          service,
          costType,
          value,
                  ...
                  {users[0]?.services[0]?.services &&
                    Object.keys(users[0].services[0].services).map(
                      (service) => (
                        <th
                          key={service}
                          className="px-6 py-3 text-left border border-gray-600"
                          colSpan={2}
                        >
                          {service}
                        </th>
                    ...
                    Object.keys(users[0].services[0].services).map(
                      (service) => [
                        <td
                          ...
                          className="px-6 py-3 text-center border border-gray-600"
                        >
                          0.5
                        </td>,
                        <td
                          key={`${service}-static-manifested`}
                          className="px-6 py-3 text-center border border-gray-600"
                        >
                          0.9
                        </td>,
              ...
              <tbody>
                {users.map((user) => (
                  <tr key={user._id} className="hover:bg-black">
                    ...
                    {Object.entries(user.services[0]?.services || {}).map(
                      ([service, prices]) => [
                        <td
                          key={`${user._id}-${service}-standard`}
                        ...
                        <td
                          key={`${user._id}-${service}-manifested`}
              ...
              <tbody>
                {users.map((user) => (
                  <tr key={user._id} className="hover:bg-black">
                    ...
                    {Object.entries(user.services[1]?.services || {}).map(
                      ([service, prices]) => [
                        <td
                          key={`${user._id}-${service}-standard`}
                          className="px-6 py-4 border border-black"
